## CODE TO FIT BASELINE SILM (ASSUMES NO BC EFFECT) TO BC DATA SETS

# Run tasks go from 1-24
...
########### FITTING BASELINE SILM (ASSUMES NO BC) TO MODEL 1A DATA ##############
...
if run_task == 1
    ...
    @everywhere begin
        ...
        function lik_pop(param2, tmax2) 
            likelihood_base(popi.inftime, popi.rem_time, tmax2, param2, popi.x, popi.y, num_ind)
        end
    ...
    process_chains(chains1, chains2, chains3, lik_pop_full, sim_data_base, n_iter, n_burn, n_hpd, mod_gen, mod_fit, scen, pop_task, tmax)


    # RUN MCMC ON FIRST 8 TIME POINTS THEN FORECAST
    chains1, chains2, chains3 = pmap(x -> get_chain_forecast(),1:Nchains)

    process_forecasts(chains1, chains2, chains3, lik_pop_forecast, sim_data_base, forecast_base, n_iter, n_burn, n_hpd, mod_gen, mod_fit, scen, pop_task, tmin_f, tmax_f)
